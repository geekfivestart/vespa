...
PackedMappingsBuilder::build_mappings() const
{
    ...
    size_t total_size = meta_size + int_store_size + label_offsets_size + label_bytes;

    char * mem = (char *)operator new(total_size);
    uint32_t * int_store_mem = (uint32_t *) (mem + meta_size);
    uint32_t * offsets_mem = (uint32_t *) (mem + meta_size + int_store_size);
    char * labels_mem = mem + meta_size + int_store_size + label_offsets_size;
    ...
    PackedMappings * built = new (mem) PackedMappings(_num_dims, _mappings.size(), int_store_data, stored_labels);
